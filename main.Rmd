...
Hay 7 campos claramente diferenciados: \* Host: Muestra quin ha hecho la petición, sería una variable cualitativa y sus valores van de direcciones IP a direcciones web. \* Timestamp: Muestra cuando se hizo la petición, en específico muestra el dia, hora, minutos y segundos. \* Method: Muestra cual ha sido el método de la petición. Es una variable cualitativa y los valores que tiene son GET, POST y HEAD. \* Resource: Muestra cuál es el recurso usado en la petición. Sería una variable cualitativa y sus valores muestras distintos tipos de ficheros. \* Protocol: Muestra el protcolo de la petición. Sería una variable cualitativa y en este caso solo prende dos valores HTTP/1.0 y HTTP/0.2 \* Status: Muestra el código del resultado de la petición. Es una variable cualitativa y sus valores son distintos códigos de status como 200, 404, etc. \* Size: Muestra el tamaño de la petición. Es una variable cuantitativa y sus valores van de 0 ha números bastante grandes.
...
Para esta version hemos creado un data frame que tiene como variable el tipo de fichero del recurso. A este data frame hemos dejado solo las entradas las cuáles el recurso era una imagen y hemos repetido el mismo proceso del apartado anterior.
...
Este histograma muestra la frecuencia del tamaño de las peticiones. Como había algunas peticiones con size indeterminado estos no han sido considerados para la creación del histograma. De aquí se puden ver dos cosas intereantes, la primera es la alta frecuencia de metodos con tamaño "pequeño" en comparación con todo el resto. La segunda es como hay pocas peticiones que tienen un tamaño exageradamente grande en comparación con el resto de peticiones, miden casi el doble.
...
-   Dar un valor a los `NA`, cómo 0 o el valor medio del dataset.
...
Optional: Además, aplicaremos la normalización min-max. Esto podría servir para que los valores de size no tengan una influencia desmesurada sobre el resto de componentes (que tienen magnitudes más pequeñas). 

```{r}

# df_km= DataFrame K Means
df_km <- df %>% drop_na()
...
Ahora processamos **method** y **status**. Para transformarlos a datos numéricos usaremos one hot encoding. Hacemos lo mismo para la columna **type** y descartamos as columnas que no necessitamos.
...
Generamos los gráficos con un boxplot. Un boxplot nos permitirá visualizar la distribución de `size`. 
...
df_current <- df %>% select(one_of("status"))
...
for (j in 1:len_clusters) {
  ...
  for (i in 1:k) {
    ...
    current$status <- droplevels(current$status)
    
    graph <- ggplot(current, aes(x = status, fill = status)) + 
      geom_bar() + 
      scale_y_log10() + 
      scale_fill_manual(values = palette_2) +
      labs(x = label, y = "Count", fill = "status") 
      # theme_minimal()
      # theme(legend.position = "top")
    # graph <- ggplot(df_current, aes(x = status, fill = palette_2)) + 
    #   scale_y_log10() + 
...
Podemos ver que siempre hay un cluster que contiene una mayor variabilidad de status code (grupo 1 para k = 2, grupo 2 para k = 3 y grupo 1 para k = 5). También suele haver un grupo que contiene el resto de códigos 302 y algunos 200. Si hay, el resto de grupos sólo hay cosigos 200. Para el caso de k = 5, eso puede significar que nos sobran grupos o que k means está capturando información que no podemos visualizar desde esta perspectiva. 

Replicamos el proceso nuevamente para **method**. 

```{r} 

palette_3 <- c("#2991F0", "#F0362B", "#E0F018")
total_graphs <- list()

df_current <- df %>% select(one_of("method"))
...
for (j in 1:len_clusters) {
  ...
  for (i in 1:k) {
    ...
    current$status <- droplevels(current$method)
    
    graph <- ggplot(current, aes(x = method, fill = method)) + 
      geom_bar() + 
      scale_y_log10() + 
      scale_fill_manual(values = palette_3) +
      labs(x = label, y = "Count", fill = "method") 
...
Podemos ver que k means siempre deja todas las peticiones de tipo POST en un único grupo (k = 2 => grupo 1, k = 3 => grupo 2, k = 5 => grupo 1). El grupo donde dejalas peticiones post coincide con el grupo dónde deja más status codes en la sección anterior, así que podemos crear la conjetura de que las peticiones post devuelven los códigos no presentes en los otros gráficos, lo que concuerda con la información del ejercicio 6. 

Sin embargo, no podemos ver muchos más diferencias entre los grupos, ya que todos parecen tener *aproximadamente* la misma proporción de requests GET y HEAD. 
