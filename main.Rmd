...
* Host: Muestra quin ha hecho la petición, sería una variable cualitativa y sus valores van de direcciones IP a direcciones web.
* Timestamp: Muestra cuando se hizo la petición, en específico muestra el dia, hora, minutos y segundos.
* Method: Muestra cual ha sido el método de la petición. Es una variable cualitativa y los valores que tiene son GET, POST y HEAD.
* Resource: Muestra cuál es el recurso usado en la petición. Sería una variable cualitativa y sus valores muestras distintos tipos de ficheros.
* Protocol: Muestra el protcolo de la petición. Sería una variable cualitativa y en este caso solo prende dos valores HTTP/1.0 y HTTP/0.2
* Status: Muestra el código del resultado de la petición. Es una variable cualitativa y sus valores son distintos códigos de status como 200, 404, etc.
* Size: Muestra el tamaño de la petición. Es una variable cuantitativa y sus valores van de 0 ha números bastante grandes.
...
# one_hot_type <- model.matrix(~ type - 1, df)
# one_hot_type <- as.data.frame(one_hot_type) 
...
k <- 3
results <- kmeans(df, centers = k, nstart = 25)
...
for (i in 1:k) {
  current = df[df$group == i]
  label = sprintf("From group %d, is an image?", i)
  boxplot(size ~ is_image, data = df, xlab = label, col = palette[i])
}
