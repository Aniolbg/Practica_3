...
df <- one_hot(df, cols = "method")

df <- one_hot(df, cols = "status")

df <- one_hot(df, cols = "type")


df <- df %>% select(-one_of('host', 'timestamp', 'resource', 'resource_1', 'protocol'))
...
results <- lapply(clusters, function(k) kmeans(df, centers = k, nstart = 25))

```

```{r, cache=TRUE}

# results_2 <- kmeans(df, centers = 2)
# typeof(results[[1]]$cluster)
```
...
```{r}
df_current <- df %>% select(one_of("size", "is_image"))
