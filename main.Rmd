---
title: "Main"
author: "Aniol Boladeras, Luis Omar Castro, Marcel Aranich"
date: "2026-01-19"
...
Los datos analizados son entradas de logs que muestran la información de peticiones hechas en lo que puede ser una página o aplicación web.
...
El algoritmo de K-menas necessita usar datos numéricos, así que tenemos que transformar las columnas **resources**, **size**, **method** y **status** a datos numéricos.
...
Ahora que tenemos un tipo púramente numérico, podemos trabajar con él. Si miramos algunos de los datos podemos ver que los datos abarcan múltiples órdenes de magnitud, por lo que tiene sentido procesarlos en una escala logarítmica, particularmente sabiendo que solo hay datos no-negativos.
...
Generamos los gráficos con un boxplot, que nos permitirá visualizar la distribución de `size`.
...
Podemos ver que siempre hay un cluster que contiene una mayor variabilidad de status code (grupo 1 para k = 2, grupo 2 para k = 3 y grupo 1 para k = 5). También suele haber un grupo que contiene el resto de códigos 302 y algunos 200. Si hay, el resto de grupos sólo hay códigos 200. Para el caso de k = 5, eso puede significar que nos sobran grupos o que k means está capturando información que no podemos visualizar desde esta perspectiva.
...
Podemos ver que k means siempre deja todas las peticiones de tipo POST en un único grupo (k = 2 =\> grupo 1, k = 3 =\> grupo 2, k = 5 =\> grupo 1). El grupo donde deja las peticiones post coincide con el grupo dónde deja más status codes en la sección anterior, así que podemos crear la conjetura de que las peticiones post devuelven los códigos no presentes en los otros gráficos, lo que concuerda con la información del ejercicio 6.
